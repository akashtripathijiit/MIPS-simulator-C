...
struct buffer_node
{
	...
	buffer_node(instruction i1, int d, int r)
	{
		dest = d;
		result = r;
		i = i1;
...
public:
	...
	void insert(instruction i2, int d, int r)
	{
		buffer_node *b = new buffer_node(i2, d, r);
		if(headnew == NULL)
		{
			headnew = b;
		}
	...
	if(pre_alu->head != NULL)
	{
		*i = pre_alu->pop(0);
		if(!strcmp(i->getcat(),"000"))
		{
			...
			else if(!strcmp(inst->operation,"SW"))
			{
...
inst_calc * process(instruction ins)
{
	...
	{
		category = 1;
		inst->arg1 = new char[2];
		inst->arg2 = new char[2];
		inst->arg3 = new char[2];
		j=0;
		i++;
		while(temp[i]!=',')
		{	
